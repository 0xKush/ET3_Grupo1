...
class USER_Model {
    ...
    public function add(Guest $guest) {
        ...
        $sql->execute(array($guest->getMember(), $guest->getEvent(), $guest->getSecondaryMember(), $guest->getStatus()));
    }
    ...
    public function guestExists($event, $member, $secondarymember)
    {
        ...
        $sql->execute(array($event, $member, $secondarymember));
        ...
        $sql = $this->db->prepare("SELECT count(id) FROM guest where event=? AND member=? AND secondarymember=?");
        $sql->execute(array($event, $secondarymember, $member));
